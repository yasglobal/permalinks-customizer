...
class Permalinks_Customizer_Update_Taxonomy {
	 ...
	 */
	private static function update_taxonomy_terms() {
		$taxonomy_table = get_option( 'permalinks_customizer_table', -1 );
		if ( isset( $taxonomy_table ) && ! empty( $taxonomy_table ) && is_array( $taxonomy_table ) ) {
			global $wpdb;
			$i = 1;
			foreach ( $taxonomy_table as $link => $info ) {
				...
				if( isset( $check_term->meta_value ) && ! empty( $check_term->meta_value ) ) {
			...
			update_option( 'permalinks_customizer_plugin_version', '1.3.6' );
		}
	}
}